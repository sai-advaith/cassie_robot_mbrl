...
class CassieEnv:
  def __init__(self, dynamics_randomization=False):
    self.sim = CassieSim("/home/advaith/cassie_project/simulator/cassie/cassiemujoco/cassie.xml")
    self.vis = None
  ...
  def step(self, action, freeze_phase = False):
      ...
      reward, contact_information = self.compute_reward()

      if reward < 0.3:
          done = True

      return self.get_full_state(), reward, done, self.phase, contact_information
    #   return self.get_full_state(), reward, done, self.phase
  ...
  def reset(self):
    ...
    #   self.speed        = np.random.uniform(-0.15, 0.8)
      ...
      contact_forces    = self.sim.contact()
      
      return self.get_full_state(), contact_forces
  ...
  def compute_reward(self):
      ...
      for i, j in enumerate(self.pos_idx):
          target = ref_pos[j]
          actual = qpos[j]
          # print(f"component {j} has value: {actual}")
      ...
      # Compare standing position
      cassie_state = self.get_full_state()
      reference_height = ref_pos[2]
      robot_pelvis_position = cassie_state[0]
      standing_error = 10 * ((reference_height - robot_pelvis_position) ** 2)
      ...
      for i in [15, 29]:
          target = ref_pos[i]
          actual = qpos[i]
          # print(f"component {i} has value: {actual}")
  ...
  def get_full_state(self):
      ...
      robot_state = np.concatenate([
          ...
          self.cassie_state.joint.position[:],                                       # unactuatoed joint psitions (34:40)
