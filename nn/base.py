...
class Net(nn.Module):
  ...
  def normalize_state(self, state, update=True):
    ...
    if torch.cuda.is_available():
      self.state_mean = self.state_mean.cuda()
      self.state_mean_diff = self.state_mean_diff.cuda()
  ...
  def init_hidden_state(self, batch_size=1):
    ...
    if torch.cuda.is_available():
      for i in range(len(self.hidden)):
        self.hidden[i] = self.hidden[i].cuda().double()
        self.cells[i] = self.cells[i].cuda().double()
